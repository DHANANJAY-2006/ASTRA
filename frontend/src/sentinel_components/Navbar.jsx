...
export default function Navbar({
  ...
  return (
    <header className="border-b border-theme-accent bg-[#080c16]/95 backdrop-blur sticky top-0 z-50 px-5 py-2.5 min-h-[60px] flex items-center justify-between shadow-lg gap-4">
      {/* Brand & Circular Logo */}
      <div className="flex items-center gap-3 shrink-0">
        <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-theme-accent shadow-theme-glow flex items-center justify-center shrink-0 bg-white p-0.5">
          <img
            src="/logo.png"
            alt="ASTRA Logo"
        ...
        <div className="flex flex-col justify-center">
          <div className="flex items-center gap-2.5">
            <span className="font-mono font-black text-lg tracking-wider text-slate-100 uppercase leading-none whitespace-nowrap">
              PROJECT <span className="text-theme-accent">ASTRA</span>
            </span>
            <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-red-950/80 text-red-400 border border-red-800/80 font-bold tracking-widest leading-none whitespace-nowrap">
              RESTRICTED // NTRO
            </span>
          </div>
          <div className="text-[11px] font-mono text-slate-400 flex items-center gap-2 whitespace-nowrap mt-1 leading-none">
      ...
      <div className="hidden 2xl:flex items-center gap-3 px-3.5 py-1.5 rounded-full bg-[#0b1324]/90 border border-slate-800 text-xs font-mono shrink-0 shadow-inner">
      ...
      <div className="flex items-center gap-2.5 shrink-0">
        ...
        <div className="flex items-center gap-1.5 bg-[#0b1220] border border-slate-800 rounded-lg px-2.5 py-1 text-xs shrink-0">
          <Palette className="w-3.5 h-3.5 text-slate-400" />
          <span className="text-[10px] font-mono text-slate-400 hidden sm:inline uppercase font-bold tracking-wide">Theme:</span>
          <div className="flex items-center gap-1">
            {THEMES.map((t) => (
              <button
                key={t.id}
                onClick={() => onThemeChange && onThemeChange(t.id)}
                className={`px-2 py-0.5 rounded text-[11px] font-mono font-bold transition flex items-center gap-1 whitespace-nowrap ${
                  activeTheme === t.id
        ...
        {/* Live Clock */}
        <div className="hidden lg:flex items-center gap-1.5 bg-[#0b1220] border border-slate-800/80 rounded-lg px-2.5 py-1 text-slate-300 font-mono text-xs shrink-0 whitespace-nowrap">
        ...
        {onTogglePitchHud && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-slate-950 font-bold text-xs font-mono transition shadow-[0_0_10px_rgba(245,158,11,0.3)] whitespace-nowrap shrink-0"
        ...
        {onToggleVideoModal && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800/90 hover:bg-slate-700 text-theme-accent border border-theme-accent font-bold text-xs font-mono transition whitespace-nowrap shrink-0"
